...
let achievements = {
  ...
  STREAK_180: {
    title: "このシリーズ忘れてたでしょ",
    description: "180日連続で勉強した",
    unlocked: false },

  STREAK_365: {
    title: "連続１年記念",
    description: "365日連続で勉強した",
    unlocked: false },

  DAY_60: {
    title: "まずはここから",
    description: "1日に60分以上勉強した",
    unlocked: false },
  ...
  DAY_180: {
    title: "凄まじい集中力",
    description: "1日に180分以上勉強した",
    unlocked: false },

  DAY_240: {
    title: "暇なの？",
    description: "1日に240分以上勉強した",
    unlocked: false },

  DAY_300: {
    title: "気分転換に散歩でも",
    description: "1日に300分以上勉強した",
    unlocked: false },

  MULTI_2: {
    title: "同じことばっかりじゃつまらない",
    description: "1日に2教科勉強した",
    unlocked: false },

  MULTI_3: {
    title: "気分転換に他の教科をやろう",
    description: "1日に3教科勉強した",
    unlocked: false },

  MULTI_4: {
    title: "いろいろやりすぎてあたまがこんがらがる",
    description: "1日に4教科勉強した",
    unlocked: false },

  MULTI_5: {
    title: "時間の使い方が上手",
    description: "1日に5教科勉強した",
    unlocked: false },

  MULTI_6: {
    title: "１教科目は何だった？",
    description: "1日に6教科勉強した",
    unlocked: false },

  FIRST_TIMER: {
    title: "これは便利かも",
    description: "タイマーを初めて使った",
    unlocked: false },

  POMODORO_1: {
    title: "ポモドーロてなんなんだ？",
    description: "ポモドーロタイマーを1回完了した",
    unlocked: false } };
...
for (let i = 0; i < levelTitles.length; i++) {
  const lv = (i + 1) * 10; // 10,20,...,100
  const key = `LEVEL_${lv}`;
  achievements[key] = {
    title: levelTitles[i],
    description: `lv${lv}になった！`,
    unlocked: false };
...
// ===== 教科マスター実績を生成 =====
const subjectMasterLevels = [100, 500, 1000, 5000, 10000];

function ensureSubjectAchievements() {
  subjects.forEach(subject => {
    subjectMasterLevels.forEach(min => {
      const key = `SUBJECT_${subject}_${min}`;

      if (!achievements[key]) {
        achievements[key] = {
          title: `${subject}を極めるもの Lv${subjectMasterLevels.indexOf(min) + 1}`,
          description: `${subject}を累計${min}分勉強した`,
          unlocked: false };
...
//ストリーク計算
function calcStudyStreak(records) {
...
function getTodayStudyMinutes() {
  ...
  return records.
  filter(r => sameDate(r.date, today)).
  reduce((sum, r) => sum + r.time, 0);
}
function getTodaySubjectCount() {
  const today = new Date();

  const todaySubjects = records.
  filter(r => sameDate(r.date, today)).
  map(r => r.subject);

  return new Set(todaySubjects).size;
}
...
function unlockAchievement(key) {
  ...
  // 実績ページを開いてたら即更新
  if (achievementPage.style.display === "block") {
    renderAchievements();
  }
}
// ===== 教科ごとの累計分数を取得 =====
function getSubjectTotalMinutes(subject) {
  return records.
...
function checkAchievements() {
  ...
  { key: "TIME_1H", minutes: 60 },
  { key: "TIME_3H", minutes: 180 },
  { key: "TIME_10H", minutes: 600 },
  { key: "TIME_24H", minutes: 1440 },
  { key: "TIME_100H", minutes: 6000 }];
  ...
  if (streak >= 4) unlockAchievement("STREAK_4");
  if (streak >= 7) unlockAchievement("STREAK_7");
  if (streak >= 30) unlockAchievement("STREAK_30");
  if (streak >= 180) unlockAchievement("STREAK_180");
  if (streak >= 365) unlockAchievement("STREAK_365");
  //１日の最大
  const todayMinutes = getTodayStudyMinutes();
  if (todayMinutes >= 60) {
    unlockAchievement("DAY_60");
  }
  if (todayMinutes >= 120) {
    unlockAchievement("DAY_120");
  }
  if (todayMinutes >= 180) {
    unlockAchievement("DAY_180");
  }
  if (todayMinutes >= 240) {
    unlockAchievement("DAY_240");
  }
  if (todayMinutes >= 300) {
    unlockAchievement("DAY_300");
  }
  const subjectCount = getTodaySubjectCount();

  if (subjectCount >= 2) unlockAchievement("MULTI_2");
  if (subjectCount >= 3) unlockAchievement("MULTI_3");
  if (subjectCount >= 4) unlockAchievement("MULTI_4");
  if (subjectCount >= 5) unlockAchievement("MULTI_5");
  if (subjectCount >= 6) unlockAchievement("MULTI_6");
  // ===== レベル到達チェック =====
  for (let lv = 10; lv <= 100; lv += 10) {
    const key = `LEVEL_${lv}`;
    if (!achievements[key].unlocked && level >= lv) {
      unlockAchievement(key);
    }
  }

  // タイマー初使用
  if (!achievements.FIRST_TIMER.unlocked && timerStart) {
    unlockAchievement("FIRST_TIMER");
  }

  // ポモドーロ1回
  if (
  !achievements.POMODORO_1.unlocked &&
  pomodoroCount >= 1)
  {
    unlockAchievement("POMODORO_1");
  }
  subjects.forEach(subject => {
    const total = getSubjectTotalMinutes(subject);
...
document.getElementById("to-achievement").onclick = () => {
...
document.getElementById("back-from-achievement").onclick = () => {
...
function renderAchievements() {
  ...
  Object.values(achievements).forEach(a => {
    ...
    title.textContent = a.unlocked ? a.title : "？？？";
  ...
  if (timerMode === "pomodoro") {
    ...
    if (remain <= 0) {
      ...
      if (Notification.permission === "granted") {
        new Notification(
        pomodoroPhase === "work" ? "集中タイムスタート！" : "休憩タイムスタート！", {
          body: pomodoroPhase === "work" ? "次の25分、がんばろう 💪" : "5分休憩しよう ☕" });
...
timerBtn.onclick = () => {
  ...
  timerMode === "pomodoro" ? `${subject}（ポモドーロ）` : `${subject} 勉強中`;
